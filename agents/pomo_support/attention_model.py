...
class AttentionModel(nn.Module): # Đã đổi tên từ ESPRCTWModel
    def __init__(self, **model_params):
        super().__init__()
        self.model_params = model_params

        self.encoder = ESPRCTW_Encoder(**model_params)
        self.decoder = ESPRCTW_Decoder(**model_params)
    ...
    def pre_forward(self, reset_state):
        ...
        # Feature Concatenation đúng chuẩn ESPRCTW
    ...
    def forward(self, state):
        ...
        if state.selected_count == 0:
            # --- FIX: FORCE MULTI-START (POMO Strategy) ---
            # Chọn node 1, 2, 3 ... làm điểm bắt đầu cho mỗi trajectory
            # tensor arange [1, 2, ... pomo_size]
            selected = torch.arange(start=1, end=pomo_size + 1, device=state.BATCH_IDX.device)[None, :].expand(batch_size, pomo_size)
            ...
            if self.training or self.model_params['eval_type'] == 'softmax':
                 ...
                 # Reshape -> (Batch*Pomo, NumNodes)
                 probs_flat = probs.reshape(batch_size * pomo_size, -1)
                 selected_flat = probs_flat.multinomial(1)
                 selected = selected_flat.view(batch_size, pomo_size)
...
class ESPRCTW_Encoder(nn.Module):
    def __init__(self, **model_params):
        super().__init__()
        self.model_params = model_params
...
class ESPRCTW_Decoder(nn.Module):
    def __init__(self, **model_params):
        super().__init__()
        self.model_params = model_params
        embedding_dim = self.model_params['embedding_dim']
