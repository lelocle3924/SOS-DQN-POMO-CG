...
DRIVE_RESULTS_DIR = "results/train_20260408_093200_stage_1_50cust"
# Read the CSV file
...
# Plot 3: Baseline Variance over epochs
axes[1, 0].plot(df['epoch'], df['baseline_variance'], marker='^', linewidth=2, markersize=4, color='#3498db')
axes[1, 0].set_xlabel('Epoch', fontsize=11)
axes[1, 0].set_ylabel('Baseline Variance', fontsize=11)
axes[1, 0].set_title('Baseline Variance', fontsize=12, fontweight='bold')
axes[1, 0].grid(True, alpha=0.3)

# Plot 4: Penalty Ratio over epochs
axes[1, 1].plot(df['epoch'], df['penalty_ratio'], marker='d', linewidth=2, markersize=4, color='#9b59b6')
axes[1, 1].set_xlabel('Epoch', fontsize=11)
axes[1, 1].set_ylabel('Penalty Ratio', fontsize=11)
axes[1, 1].set_title('Penalty Ratio', fontsize=12, fontweight='bold')
...
print(f"\nBaseline Variance:")
print(f"  Initial: {df['baseline_variance'].iloc[0]:.2f}")
print(f"  Final:   {df['baseline_variance'].iloc[-1]:.2f}")
print(f"  Min:     {df['baseline_variance'].min():.2f} (Epoch {df['baseline_variance'].idxmin() + 1})")
print(f"  Max:     {df['baseline_variance'].max():.2f} (Epoch {df['baseline_variance'].idxmax() + 1})")
