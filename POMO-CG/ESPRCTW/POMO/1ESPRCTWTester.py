...
class ESPRCTWTester:
    def __init__(self,
        ...
        checkpoint_fullname = rf"{model_load['path']}\checkpoint-{model_load['epoch']}.pt"
    ...
    def _test_one_batch(self, batch_size):
        ...
        for x in range(len(decisions)):
            ...
            for y in range(len(decisions[0, :, :])):
                ...
                # Lấy node được chọn ở bước y, của bài toán x, tại 'đầu' POMO tốt nhất
                '''node_id = int(decisions[y, x, best_rewards_indexes[x]])
                if node_id == 0 and len(decision_list) > 0 and decision_list[-1] == 0:
                    continue # Bỏ qua các số 0 dư thừa ở đuôi nếu muốn
                decision_list.append(node_id)'''
            print(f"Batch {x} Route: {decision_list}")
            print("--------------")
        print("with rewards: ")
        print(best_rewards)
